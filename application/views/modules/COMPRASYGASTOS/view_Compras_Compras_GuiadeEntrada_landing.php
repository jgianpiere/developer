            ...
            <div id="myTabContent" class="tab-content">
                <div class="tab-pane fade active in" id="tab_01">
                    <div style="padding-top:15px;" >
                        <div class="row tab-pane-title col-xs-12 col-sm-12 col-md-12 col-lg-12">Registro de Guía de Entrada</div>
                            <!-- Formulario -->
                            <div class="content">
                                <div class="row">
                ...
                <div class="tab-pane fade" id="tab_02">
                    <div style="padding-top:15px;" >
                        <form class="form-horizontal" id="form_agregarOrdendeCompra" name="form_agregarOrdendeCompra">
                            <div class="row tab-pane-title col-xs-12 col-sm-12 col-md-12 col-lg-12" style="margin-bottom:30px;">Registro de Guía de Entrada</div>
                            <!-- Formulario -->
                            <div class="content">
                                <div class="row">
                                        <div class="form-group">
        ...
        $('form#form_buscarOrdendeCompra').on('submit',function(e){ $$ = this; $this = $(this); $form = $this; e.preventDefault();
            if(1==1){
                $.ajax({
                    type    : 'POST',
                    data    : $form.serialize(),
                    url     : "<?=site_url('/Compras/Compras/GuiadeEntrada/Buscar');?>",
                    success : function(data){
                        try{
                            $data = $.parseJSON(data);
                            if($data[0] != 'ERROR' && $data[0] != '00'){
            ...
            if(1==1){
                ...
                if(parseFloat($total)>= 0){
                    ...
                    $.ajax({
                        ...
                        url     : "<?=site_url('/Compras/Compras/GuiadeEntrada/Agregar');?>",
                        success : function(data){
                            try{
                                $data = $.parseJSON(data);
    ...
    (function(){
        $('[typeDocSerie]').on('change',function(){ $$ = this; $this = $(this);
            if($this.val() > 0){
                $.ajax({
                    type        : 'POST',
                    url         : "<?=site_url('/Compras/Compras/GuiadeEntrada/NumeracionDoc/');?>/"+$this.val(),
                    success     : function(data){
                        try{
                            $data = $.parseJSON(data);
                            if($data[0] != 0){
        ...
        $(document).on('click','.delete_row',function(){ $$ = this; $this = $(this);
            if(confirm('Eliminar Orden de Compra ?')){
                $.ajax({
                    ...
                    url         : "<?=site_url('/Compras/Compras/GuiadeEntrada/Eliminar');?>",
                    success     : function(data){
                        try{
                            $data = $.parseJSON(data);
                            if($data[0]!='ERROR' && $data[0] != '00'){
