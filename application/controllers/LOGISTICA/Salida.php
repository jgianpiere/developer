...
class Salida extends MY_Controller {
    ...
    */
    public function Salida_Landing(){
        if ($_SERVER['REQUEST_METHOD'] == 'POST'):

            $this->load->model('COMPRASYGASTOS/m_Compras'); // Cargar Model de Compras 

            # RutaGuia
            $rutas =   array($this->rutapadre,array('title'=>'Logistica','route'=>site_url('Logistica#/Logistica/Salida')));
            $RutaGuia = $this->htmltemplate->HTML_RutaGuia($rutas,'Salida');
            ...
            $this->load->view('modules/LOGISTICA/view_Logistica_Salida_landing.php');
    ...
    public function Salida_Buscar(){
        if ($_SERVER['REQUEST_METHOD'] == 'POST'):
            ...
            if($this->form_validation->run() == TRUE):
                ...
                $insert_result = $this->m_Logistica->Query_Buscar_CP_GuiaSalida($Params);

                $html = $this->htmltemplate->HTML_SalidaInventario($insert_result);
    ...
    *   @author     : Gianpiere Ramos Bernuy. 
    */
    public function Salida_Agregar(){
        if ($_SERVER['REQUEST_METHOD'] == 'POST'):
            $Campos = array(
            ...
            if($this->form_validation->run() == TRUE):
               ...
               $agre_modalidad_OrdenCompra          = $this->input->post('agre_modalidad_OrdenCompra'); 
                ...
                $insert_result = $this->m_Logistica->Query_Insertar_CP_GuiaSalida($Params);
                    ...
                    if(!empty($Detalles)):
                        ...
                        if(!empty($Detalles) && is_array($Detalles)):
                            ...
                            foreach ($Detalles as $key => $value) {
                                ...
                                $rslt = $this->m_Compras->Query_Insertar_Detalle_CP_GuiaSalida($Params_detalle);
